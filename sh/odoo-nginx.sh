...
server {
    ...
    # Realtime bus (Discuss/LiveChat) — Odoo 16+/18 use /websocket (not /longpolling)
    # IMPORTANT: catch both /websocket and /websocket/ to avoid 301 redirects breaking WS upgrade
    location ^~ /websocket {
...
# Replace placeholders the heredoc couldn't see (domain/ports/log names)
# (The heredoc above is single-quoted to preserve \$nginx_vars; we substitute our bash vars here.)
sed -i \
  -e "s/\${DOMAIN}/${DOMAIN}/g" \
