#!/usr/bin/env bash
# odoo-nginx.sh — Create an Nginx server block for Odoo (reverse-proxy), HTTP-only
...
# ----- write config (single-quoted heredoc keeps $host/$scheme intact) -----
cat > "$CONF_PATH" <<'NGINX'
...
server {
    ...
    # Allow Certbot (webroot plugin) validation later
    ...
    location / {
        ...
        proxy_set_header X-Forwarded-Proto $scheme;   # http (for now)
    ...
    # ---- Realtime bus (Discuss/LiveChat) ----
    # Catch BOTH /websocket and /websocket/ to avoid redirects breaking WS upgrade
    location ^~ /websocket {
        proxy_pass http://${KEY}_longpolling;   # gevent/bus upstream
        ...
        proxy_read_timeout 86400s;   # keep long-lived ws connections
        proxy_send_timeout 86400s;
        proxy_buffering off;         # do not buffer websocket frames
    }
...
# Replace placeholders the heredoc left literal
sed -i \
  -e "s/\${DOMAIN}/${DOMAIN}/g" \
  -e "s/\${HTTP_PORT}/${HTTP_PORT}/g" \
  -e "s/\${GEVENT_PORT}/${GEVENT_PORT}/g" \
  -e "s/\${KEY}/${KEY}/g" \
